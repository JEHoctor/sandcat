...
# Apply DNS configuration to a resolv.conf-style file.
#
# If $1 (a dns.conf written by the mitmproxy addon) exists and contains at
# least one non-blank line, those entries are used as the `nameserver` lines.
# Otherwise the hardcoded defaults are used.
#
# dns.conf is written exclusively by the addon — one IP per line, no comments
# or padding — so the parser only does the minimum needed to be robust against
# accidental empty lines.
#
# Args:
#   $1 - path to dns.conf (sidecar from mitmproxy)
#   $2 - path to resolv.conf to overwrite
apply_dns_config() {
    local dns_conf="$1"
    local resolv_conf="$2"
    local nameservers=()
...
main() {
    ...
    # The mitmproxy addon writes $DNS_CONF when the user configures
    # `dns_servers` in settings.json (e.g. corporate/intranet DNS); otherwise
    # we fall back to a generic public resolver.
    apply_dns_config "$DNS_CONF" /etc/resolv.conf
