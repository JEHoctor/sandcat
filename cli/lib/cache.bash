...
sct_cache_volume_names() {
	sct_engine_run volume ls -q --filter label=sandcat-shared-cache=true 2>/dev/null | sort
}
...
sct_cache_volume_size_bytes() {
	local name=$1
	sct_engine_run run --rm -v "$name":/mnt alpine du -sb /mnt 2>/dev/null | awk '{print $1}'
}
...
sct_cache_volume_file_count() {
	local name=$1
	sct_engine_run run --rm -v "$name":/mnt alpine sh -c 'find /mnt -type f 2>/dev/null | wc -l' | awk '{print $1}'
}
...
sct_cache_volume_containers() {
	local name=$1
	sct_engine_run ps -a --filter volume="$name" --format '{{.Names}}' 2>/dev/null
}
